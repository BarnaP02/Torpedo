...
void salvo(Game* g, int* p1intact, int* cp2intact, int* cpmiss, int pov, bool* if_hit, int* damaged_ship, int* last_col, int* last_row, int* first_hit_col, int* first_hit_row, int* align, int* palette, int* global) {
    //printf("pov%d\n", pov);
    if (*global == 7) return;
    bool ship_ended = false;        //azért kell hogy ne legyen rekurzív a cpfire   //akkor lesz true amikor visszaugrik az elsõ találat koordinátára a cpfire anélkül hogy lõne
    if (pov == 1)
    {
        system("cls");
